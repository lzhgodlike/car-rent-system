...
const loadData = async () => {
  loading.value = true
  try {
    const page = await request.get('/return-orders', { params: { pageNum: currentPage.value, pageSize: pageSize.value } })
    ...
    <div class="card" style="margin-top:16px;">
      <el-table :data="records" v-loading="loading">
        ...
        <el-table-column label="操作" width="100">
          <template #default="{row}"><el-button v-if="row.status === 'PENDING'" size="small" type="primary" @click="openConfirm(row)">确认还车</el-button></template>
        </el-table-column>
      </el-table>
