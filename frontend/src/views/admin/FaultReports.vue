...
const loadData = async () => {
  loading.value = true
  try {
    const page = await request.get('/fault-reports', { params: { pageNum: currentPage.value, pageSize: pageSize.value } })
...
const completeRepair = async (row) => {
  await request.put(`/fault-reports/${row.id}/complete-repair`)
  ElMessage.success('维修完成'); loadData()
}
      ...
      <el-table :data="records" v-loading="loading">
        ...
        <el-table-column label="操作" width="200">
          <template #default="{row}">
            <el-button v-if="row.faultStatus === 'PENDING'" size="small" type="primary" @click="openHandle(row)">处理</el-button>
            <el-button v-if="row.faultStatus === 'PENDING'" size="small" type="info" plain @click="rejectFault(row)">拒绝</el-button>
            <el-button v-if="row.faultStatus === 'REPAIRING'" size="small" type="success" @click="completeRepair(row)">完成维修</el-button>
          </template>
        </el-table-column>
      </el-table>
